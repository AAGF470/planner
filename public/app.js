...
const S = {
  ...
  ctxTargetId: null,
  editingNodeId: null, editingProjectId: null,
  modalColor: '#1e6fff', projectModalColor: '#1e6fff',
...
async function loadBoard() {
  ...
  } catch(e) { updateSaveStatus('local') }
}
...
function drawGrid() {
  ...
  ctx.lineWidth = 1
  for (let x = offX; x < W; x += spacing) { ctx.beginPath(); ctx.setLineDash([1, spacing-1]); ctx.moveTo(x,0); ctx.lineTo(x,H); ctx.stroke() }
  for (let y = offY; y < H; y += spacing) { ctx.beginPath(); ctx.setLineDash([1, spacing-1]); ctx.moveTo(0,y); ctx.lineTo(W,y); ctx.stroke() }
  ctx.setLineDash([])
...
function initEdgeSVG() {
  ...
  defs.innerHTML = `
    <marker id="arr" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0,8 3,0 6" fill="#2a3f5a"/></marker>
    <marker id="arr-sel" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto"><polygon points="0 0,8 3,0 6" fill="#1e6fff"/></marker>
  `
  svg.appendChild(defs)
  const dp = document.createElementNS('http://www.w3.org/2000/svg','path')
  dp.id='drag-edge'; dp.style.display='none'; dp.setAttribute('fill','none')
  dp.setAttribute('stroke','#f59e0b'); dp.setAttribute('stroke-width','1.5'); dp.setAttribute('stroke-dasharray','5 4')
  svg.appendChild(dp)
...
function createNodeEl(node) {
  ...
  const dateStr = (node.startDate||node.endDate) ? [node.startDate,node.endDate].filter(Boolean).map(fmtDate).join(' → ') : ''
  el.innerHTML = `
  ...
  el.addEventListener('mousedown', e => {
    ...
    if (e.target.classList.contains('node-port')) { startConnection(node.id,e); return }
  ...
  el.addEventListener('contextmenu', e => { e.preventDefault(); e.stopPropagation(); S.ctxTargetId=node.id; showCtxMenu(e.clientX,e.clientY) })
  return el
}
...
function renderEdges() {
  ...
  S.edges.forEach(edge => {
    ...
    hit.setAttribute('d',d); hit.setAttribute('fill','none'); hit.setAttribute('stroke','transparent'); hit.setAttribute('stroke-width','12'); hit.style.cursor='pointer'
    hit.addEventListener('click',ev=>{ev.stopPropagation();S.selectedEdge=edge.id;S.selectedNodes.clear();updateSelectionVisuals()})
    hit.addEventListener('contextmenu',ev=>{ev.preventDefault();if(confirm('Delete this connection?')){S.edges=S.edges.filter(e=>e.id!==edge.id);renderEdges();scheduleSave()}})
    g.appendChild(hit)
    const path = document.createElementNS('http://www.w3.org/2000/svg','path')
    path.className='edge-path'+(isSel?' selected':''); path.setAttribute('d',d); path.setAttribute('marker-end',isSel?'url(#arr-sel)':'url(#arr)')
    g.appendChild(path)
    if (edge.label) {
      const mx=(x1+x2)/2, my=(y1+y2)/2
      const txt=document.createElementNS('http://www.w3.org/2000/svg','text')
      txt.className='edge-label'; txt.setAttribute('x',mx); txt.setAttribute('y',my-5); txt.setAttribute('text-anchor','middle'); txt.textContent=edge.label
      g.appendChild(txt)
    }
    svg.appendChild(g)
  })
...
function cancelConnection() {
  if (S.connectingFrom) { document.getElementById('node-'+S.connectingFrom)?.classList.remove('connecting-source'); S.connectingFrom=null }
...
function initEventListeners() {
  ...
  wrap.addEventListener('mousedown', e => {
    ...
    const onCanvas = e.target===wrap||e.target.id==='grid-canvas'||e.target.id==='board'||e.target.id==='edge-svg'
    if (onCanvas) {
      // Click-to-place a pending node
      if (S.pendingNodeData) { placeNodeAtCursor(e); return }
      if (S.connectingFrom) { cancelConnection(); return }
      S.selectedNodes.clear(); S.selectedEdge=null; updateSelectionVisuals(); closeCtxMenu()
      startPan(e)
    }
  })
  ...
  wrap.addEventListener('mousemove', e => {
    ...
    // Show crosshair when a node is pending placement
  ...
  wrap.addEventListener('wheel', e => {
    e.preventDefault()
    const delta=e.deltaY>0?0.9:1.1, newZoom=Math.max(0.15,Math.min(2.5,S.zoom*delta))
    const rect=wrap.getBoundingClientRect(), mx=e.clientX-rect.left, my=e.clientY-rect.top
    S.panX=mx-(mx-S.panX)*(newZoom/S.zoom); S.panY=my-(my-S.panY)*(newZoom/S.zoom); S.zoom=newZoom
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.code==='Space') { e.preventDefault(); activateTool('pan') }
    if (e.key==='v'||e.key==='V') activateTool('select')
    if (e.key==='c'||e.key==='C') activateTool('connect')
    if (e.key==='f'||e.key==='F') fitView()
    if ((e.key==='Delete'||e.key==='Backspace')&&!e.target.closest('.modal')) deleteSelected()
    if (e.key==='Escape') { cancelConnection(); closeCtxMenu(); closeDetailPanel(); closeProjectsPanel(); cancelPendingNode(); activateTool('select') }
    if (e.key==='n'||e.key==='N') openNodeModal()
  })
  document.addEventListener('keyup', e => { if(e.code==='Space') activateTool('select') })
...
function dragNode(e) {
  ...
  const dx=(e.clientX-S.draggingNode.mouseX)/S.zoom, dy=(e.clientY-S.draggingNode.mouseY)/S.zoom
  node.x=S.draggingNode.startX+dx; node.y=S.draggingNode.startY+dy
  const el=document.getElementById('node-'+node.id)
  if(el){el.style.left=node.x+'px';el.style.top=node.y+'px'}
  renderEdges()
}
function startPan(e) { S.isPanning=true; S.panStart={x:e.clientX-S.panX,y:e.clientY-S.panY} }
function doPan(e)    { S.panX=e.clientX-S.panStart.x; S.panY=e.clientY-S.panStart.y; applyViewport(); renderEdges() }
...
// ── Tool management ───────────────────────────────────────────────
// setTool is the PUBLIC button handler — N key and + Node button open modal directly
function setTool(tool) {
  if (tool==='node') { openNodeModal(); return }
  activateTool(tool)
}

// activateTool sets the internal tool state without side effects
function activateTool(tool) {
  ...
  else if (tool==='connect') wrap.style.cursor='crosshair'
...
function onNodeTypeChange() {
  ...
  pf.style.flexDirection='column'; pf.style.gap='10px'
  df.style.display=t==='doc'?'flex':'none'
  df.style.flexDirection='column'; df.style.gap='10px'
}
...
function saveNodeModal() {
  ...
  if (S.editingNodeId) {
    ...
    // Enter click-to-place mode — cursor shows crosshair, click canvas to drop
    ...
    badge.textContent='Click canvas to place node — Esc to cancel'
    badge.style.background='rgba(245,158,11,0.15)'
    badge.style.color='#f59e0b'
    badge.style.borderColor='rgba(245,158,11,0.4)'
    const restore=()=>{ badge.textContent=Math.round(S.zoom*100)+'%'; badge.style.background=''; badge.style.color=''; badge.style.borderColor='' }
    S._restoreBadge=restore
  }
}
...
function openDetailPanel(nodeId) {
  ...
  document.getElementById('detail-body').innerHTML=`
    ...
    ${hasLink?`<div class="dp-section"><div class="dp-label">Document</div><a href="${esc(href)}" target="_blank" style="color:var(--accent2);font-size:12px;word-break:break-all">${esc(node.url||node.path)} ↗</a></div>`:''}
...
function ctxConnect()   { activateTool('connect'); S.connectingFrom=S.ctxTargetId; document.getElementById('node-'+S.ctxTargetId)?.classList.add('connecting-source'); closeCtxMenu() }
...
function drawMinimap() {
  ...
  ctx.clearRect(0,0,W,H); ctx.fillStyle='#090d12'; ctx.fillRect(0,0,W,H)
  if(S.nodes.length===0) return
  ...
  S.nodes.forEach(n=>{
    const proj=S.projects.find(p=>p.id===n.projectId),color=n.color||(proj?proj.color:'#1e6fff')
    ctx.fillStyle=color+'88'; ctx.fillRect(n.x*scale+offX,n.y*scale+offY,220*scale,40*scale)
  })
  const wrap=document.getElementById('canvas-wrap')
  ctx.strokeStyle='rgba(30,111,255,0.6)'; ctx.lineWidth=1
...
document.addEventListener('DOMContentLoaded',()=>{
  document.getElementById('minimap')?.addEventListener('click',e=>{
    ...
    const wrap=document.getElementById('canvas-wrap')
    S.panX=wrap.clientWidth/2-(minX+(e.clientX-rect.left)/canvas.width*bW)*S.zoom
    S.panY=wrap.clientHeight/2-(minY+(e.clientY-rect.top)/canvas.height*bH)*S.zoom
    applyViewport(); renderEdges()
  })
})
